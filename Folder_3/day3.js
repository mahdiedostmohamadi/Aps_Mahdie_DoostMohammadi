...
let destination1 = {
  ...
  */
  ...
  .map(element =>element.destinationName);
  ...
  .filter(x=>x.transportations.includes('ferry')).map(x=>x.destinationName)
  // Complete here
  
   let destinationNamesMoreThan300KmsAwayByTrain =travelDestinations
   .filter(x=>x.distanceKms>300&&x.transportations.includes('train')).map(x=>x.destinationName)
