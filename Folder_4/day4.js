...
let restaurant1 = {
  ...
  */
  ...
  findAvailableRestaurants: function (numberOfPeople) {
    // Complete here
    let availableSeats = restaurants.map((x) =>
    (x.totalSeats - x.numberOfCustomers)>numberOfPeople ? x.name : null);
    return availableSeats;
  },
  
  findRestaurantServingDish: function (dishName) {
    // Complete here
    let matchRestuarants = restaurants.map((restaurant) => {
      if (restaurant.menu.includes(dishName)) {
        return restaurant.name;
      }
    });
    return matchRestuarants;
  },
